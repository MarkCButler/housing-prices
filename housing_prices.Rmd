...
generate_box_plot <- function(variable_name, eta_squared,
    ...
    if (show_encoding) {
        fig <- fig +
            geom_point(aes(y = encoding, color = 'encoding')) +
            scale_color_manual(name = variable_name,
                               values = c('encoding' = 'red')) +
            guides(fill = FALSE)
    }
...
dummy variable.
  ...
  - Example:  A dummy variable NoBasement that identifies houses without a
    basement was defined order to avoid distortions in the fit for variables
    that characterize basement area.
...
Use one of three schemes to encode each nominal variable that explains a
significant fraction of the variance in the sale price.

#### Discussion {.unnumbered}

Three schemes for encoding categories as numbers:  one-hot, ordinal, target.

  - One-hot encoding creates dummy variables for individual categories of a
    predictor.  When many different categories are present, the increase in
    model complexity may be a problem.

  - Ordinal encoding replaces categories by natural numbers 1, 2, etc.  This
    simple encoding may not give a linear relation between predictor and
    target.

  - Target encoding encodes each category by the mean of the values for that
    category.  This method is prone to overfitting, although variants designed
    to minimize overfitting have been developed.  To minimize overfitting,
    k-fold target encoding will be used.

Method of assigning an encoding scheme to each variable:

  - Ordinal:  In cases where the profile of the box plot is roughly linear,
    categories are ordered by the mean of the sale price, and the ordering is
    used as an encoding.

  - Target:  If the central values for categories are well separated but the
    profile is not linear, the mean of each category is used as an encoding.

  - One-hot:  When most of the categories appear not to effect the sale price,
    it appears more efficient to convert each category to a dummy variable, so
    that the categories that do not significantly affect the sale price can be
    eliminated from the model.

Encoding is needed for the multilinear model but should not be needed for a
tree-based model.  Since the linear model should not be overly complex, only
predictors with $\eta^2 \geq 0.2$ were considered for encoding.
...
Changes to the variables:
  ...
  5. Each nominal variable with large $\eta^2$ was encoded using one of three
     schemes:  one-hot, ordinal, target.
...
Different methods of encoding categorical variables could be explored, with
the choice determined by cross validation for a particular model.
