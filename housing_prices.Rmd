...
for (variable_name in r_squared_changes[['original model']]) {
    ...
    r_squared <- round(r_squared_changes[bool_index, 'original R squared'],
                       digits = 3)
    linear_model <- linear_models[[variable_name]]
    plot_simple_fit(variable_name, linear_model, r_squared)
}
...
Three schemes for encoding categories as numbers:  one-hot, ordinal, target.
  ...
  1.  One-hot encoding creates dummy variables for individual categories of a
      predictor.  When many different categories are present, the increase in
      model complexity may be a problem.

  2.  Ordinal encoding replaces categories by natural numbers 1, 2, etc.
      This simple encoding may not give a linear relation between predictor and
      target.

  3.  Target encoding encodes each category by the mean of the values for that
      category.  This method is prone to overfitting, although variants designed
      to minimize overfitting have been developed.

#### $R^2$ {.tab-display .unnumbered}

<div class="code-block">

```{r, encode, cache=FALSE}


# For these predictors, the profile of the box plot is roughly linear, so
# it is natural to try ordinal encoding.
encode_ordinal <- c(
    'Neighborhood', 'ExterQual', 'KitchenQual', 'GarageFinish',
    'GarageType', 'MSSubClass', 'FireplaceQu', 'Foundation',
    'HeatingQC', 'MasVnrType', 'BsmtExposure', 'CentralAir',
    'Electrical', 'PavedDrive'
)

# The profile of the box plot shows distinct differents in the sale price
# between categories, but the profile is not linear.
encode_target <- c(
    'BsmtQual', 'Exterior1st', 'MSZoning', 'Exterior2nd',
    'GarageCond', 'GarageQual', 'SaleCondition', 'SaleType',
    'BsmtCond'
)

# From the box plot, most of the categories appear not to effect the sale price.
# For these variables, it appears more efficient to convert each category to a
# dummy variable, so that the categories that do not significantly affect the
# sale price can be eliminated from the model.
encode_one_hot <- c('BsmtFinType1', 'HouseStyle', 'LotShape')
```
...
```{r scatter-plots-encoded, cache=FALSE}
```
