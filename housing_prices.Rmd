...
in variables where it represents a valid category.
  ...
  - Example:  The description of variable BsmtExposure has "NA" indicating "No
    ...
    TotalBsmtSF ("Total square feet of basement area") is 936.
...
```{r, plot-target, cache=FALSE, echo=FALSE}
...
# values for GarageYrBlt.  The preprocessing set this variable to zero for
# houses without a garage.  For convenience in dropping data for plotting,
# temporarily replace zero by NA.
...
get_linear_model <- function(predictors) {
    ...
    # numeric predictors.
    model_formula <- as.formula(paste0('SalePrice ~ ', predictors))
...
simple_linear_models <- map(r_squared_variables, get_linear_model)
...
plot_simple_fit <- function(variable_name, prediction, r_squared = 0) {
    description <- variable_descriptions[[variable_name]]
    to_plot <- select(train_data, SalePrice, all_of(variable_name)) %>%
        drop_na() %>%
...
for (variable_name in large_r_squared$name) {
    ...
    plot_simple_fit(variable_name, prediction, r_squared)
}
```
...
### Define shared dummy variables {.tabset}

#### Summary {.unnumbered}

Certain categories, such as "No Basement", are included in multiple
predictors.  Define dummy variables for these categories.

Excluding values represented by these dummy variables from box plots and
scatter plots for other variables gives stronger linear profiles.

  - Example:  The "No Basement" category also corresponds to the intercept for
    some numeric variables, such as TotalBsmtSF (Total square feet of basement
    area).  Adding the dummy variable allows the nonzero values of TotalBsmtSF
    to be fit independently, giving a better linear association.
...
# In the r chunk 'continuous-distributions', the variable GarageYrBlt was set
# to NA in the training data for houses that have no garage.  This was done to
# to simplify commands for plotting variables inside a loop.  (All missing
# values were dropped before plotting.)  The consistency checks done during
# preprocessing found no missing values in GarageYrBlt, so NA could be used
# without ambiguity as an equivalent to 0.  Reset NA to zero in this variable.
bool_index <- is.na(train_data$GarageYrBlt)
train_data[bool_index, 'GarageYrBlt'] <- 0

# Add dummy variables to data.  The variables used in defining the new dummy
# variables have no missing values in either training data or test data.
define_dummies <- function(data) {
    data <- mutate(data,
                   NoBasement = as.integer(TotalBsmtSF == 0),
                   NoGarage = as.integer(GarageYrBlt == 0),
...
# Find the dummy variable that imposes a constraint on the values for a variable.
get_related_dummy <- function(variable_name) {
    dummy <- case_when(
...
all_predictors <- all_column_names[bool_index]
related_dummy <- get_related_dummy(all_predictors)
names(related_dummy) <- all_predictors

```

```{r, show-dummy, ref.label=c('dummy-variables', 'better-scatter-plots'), eval=FALSE}
```

```{r, better-scatter-plots, cache=TRUE, echo=FALSE}
# To see how certain fits are improved by the addition of dummy variables,
# include a dummy variable in the model for certain predictors.  For example,
# include NoBasement as a second predictor in the finding the linear fit for
# the predictor TotalBsmftSF.
for (variable_name in large_r_squared$name) {
    # Get the related dummy if there is one.  If there is, update simple_linear_prediction.
    # Or maybe define a new variable of lienar predictions.
}
```
...
### Numerical predictors {.tabset}

#### Summary {.unnumbered}

For some numerical variables, the simple linear fit would be significantly
improved if the samples with value zero were excluded from the fit.  Add dummy
variables that distinguish zero from nonzero samples for these variables.

  - Example:  TotalBsmtSF (Total square feet of basement area) can be
    augmented by a dummy variable that equal 1 when the basement area is zero.
    This allows the samples with nonzero basement area to be fit
    independently.

Drop outliers that are distorting the simple linear fits.


Replace nominal variables by numerical rankings in cases where the box plot
has a strongly linear profile.
